...
type
  ...
  public
    ...
    class function RunTest(suite: ITest; outputFile: String)
...
function RunTest(suite: ITest; outputFile: String = DEFAULT_FILENAME)
  : TTestResult; overload;
function RunRegisteredTests(outputFile: String = DEFAULT_FILENAME)
  : TTestResult; overload;
...
begin
  ...
  begin
    writeReport
      (Format('    <test-case name="%s%s" executed="%s" success="True" time="%1.3f" result="Pass"/>',
      [GetCurrentCaseName, test.GetName, TrueFalse[test.Enabled],
      test.ElapsedTestTime / 1000]));
  end;
end;
...
begin
  writeReport
    (Format('    <test-case name="%s%s" executed="%s" success="False" time="%1.3f" result="Error">',
    [GetCurrentCaseName, error.FailedTest.GetName,
    TrueFalse[error.FailedTest.Enabled], error.FailedTest.ElapsedTestTime
    / 1000]));
  writeReport('      <failure>');
  writeReport(Format('        <message><![CDATA[Exception: [%s] %s',
    [error.ThrownExceptionName, error.ThrownExceptionMessage]));
  writeReport(Format('At: %s]]></message>', [text2sgml(error.AddressInfo)]));
  writeReport(Format('        <stack-trace><![CDATA[%s]]></stack-trace>',
    [error.StackTrace]));
  writeReport('      </failure>');
...
begin
  writeReport
    (Format('    <test-case name="%s%s" executed="%s" success="False" time="%1.3f" result="Failure">',
    [GetCurrentCaseName, failure.FailedTest.GetName,
    TrueFalse[failure.FailedTest.Enabled], failure.FailedTest.ElapsedTestTime
    / 1000]));
  writeReport('      <failure>');
  writeReport(Format('        <message><![CDATA[Exception: [%s] %s',
...
begin
  ...
  writeReport
    (Format('<test-results name="%s" total="%d" not-run="%d" date="%s" time="%s">',
...
var
  ...
  successRate: Integer;
begin
  runTime := (TThread.GetTickCount - startTime) / 1000;
  if testResult.RunCount > 0 then
    successRate :=
      Trunc(((testResult.RunCount - testResult.failureCount -
      testResult.errorCount) / testResult.RunCount) * 100)
  else
    successRate := 100;

  writeReport('<statistics>' + CRLF + '<stat name="tests" value="' +
    intToStr(testResult.RunCount) + '" />' + CRLF +
    '<stat name="failures" value="' + intToStr(testResult.failureCount) + '" />'
    + CRLF + '<stat name="errors" value="' + intToStr(testResult.errorCount) +
    '" />' + CRLF + '<stat name="success-rate" value="' + intToStr(successRate)
    + '%" />' + CRLF + '<stat name="started-at" value="' +
    DateTimeToStr(dtStartTime) + '" />' + CRLF +
    '<stat name="finished-at" value="' + DateTimeToStr(Now) + '" />' + CRLF +
    Format('<stat name="runtime" value="%1.3f"/>', [runTime]) + CRLF +
    '</statistics>' + CRLF + '</test-results>');

  if TTextRec(FOutputFile).Mode = fmOutput then
    Close(FOutputFile);
end;

class function TXMLTestListener.RunTest(suite: ITest; outputFile: String)
  : TTestResult;
begin
  Result := TestFramework.RunTest(suite, [TXMLTestListener.Create(outputFile)]);
end;

class function TXMLTestListener.RunRegisteredTests(outputFile: String)
  : TTestResult;
begin
...
begin
  if CompareText(suite.Name, ExtractFileName(ParamStr(0))) = 0 then
    Exit;
  writeReport('  </results>');
...
begin
  ...
  writeReport(Format('<test-suite name="%s" total="%d" not-run="%d">',
    [s, suite.CountTestCases, suite.CountTestCases -
    suite.CountEnabledTestCases]));
...
class function TXMLTestListener.StringReplaceAll(text, byt,
  mot: string): string;
var
  plats: Integer;
begin
